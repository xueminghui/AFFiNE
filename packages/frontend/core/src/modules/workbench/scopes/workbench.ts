...
import { LiveData, Scope } from '@toeverything/infra';
...
export class Workbench extends Scope {
  readonly views$ = new LiveData([this.framework.createScope(View, nanoid())]);
  ...
  createView(at: WorkbenchPosition = 'beside', defaultLocation: To) {
    const view = this.framework.createScope(
      View,
      nanoid(),
      defaultLocation as any
    );
